...
#: §43 — terminal outcomes of an update attempt, split by whether the shop is
#: left in a good state. Everything that stopped the update is a failure, even
#: when it stopped *safely* (ABORTED = no backup, so nothing was touched).
_UPDATE_FAILURE_STATUSES = {"FAILED", "ABORTED"}


def _audit_update(db: Session, action: str, status: str, message: str,
                  latest_version: str | None = None) -> None:
    """Best-effort audit row. An audit failure must never mask the real outcome."""
    try:
        from .audit import write_audit

        write_audit(db, action=action, entity_type="System",
                    after={"status": status, "message": message,
                           "latest_version": latest_version})
    except Exception:  # pragma: no cover - defensive
        logging.getLogger("supermarket.update").exception("audit write failed: %s", action)
...
def prepare_update(
    ...
    *,
    ...
    §43 lifecycle events (UPDATE_STARTED / UPDATE_COMPLETED / UPDATE_FAILED)
    are written here rather than in the router, so a future caller — the
    desktop shell, a scheduled job, the CLI — cannot forget them.
    """
    _audit_update(db, "UPDATE_STARTED", "STARTED",
    ...
    failed = result.get("status") in _UPDATE_FAILURE_STATUSES
    _audit_update(
        db,
        "UPDATE_FAILED" if failed else "UPDATE_COMPLETED",
        result.get("status", "UNKNOWN"),
        result.get("message", ""),
        (result.get("latest") or {}).get("version") if isinstance(result.get("latest"), dict) else None,
    )
    db.commit()
    return result
