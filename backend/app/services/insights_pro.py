...
from . import pricing_law  # v3.6.1 — nothing may propose a price over the printed consumer price
...
def a_negative_margin(ctx: Ctx) -> list[Draft]:
    ...
    # v3.6.1 — "cost + 12 %" was proposed with no reference to the price printed on the pack, so
    # for anything bought dearer than its own consumer price the button offered an illegal sale.
    # The ceiling decides whether pricing is even an available answer.
    ceiling_note = ""
    if b:
        want = round(_f(b.buy_price) * 1.12 / 100) * 100
        legal = pricing_law.clamp_sell(b, want)
        if pricing_law.ceiling(b) is None:
            acts.insert(0, {"type": "set_price", "label": f"اصلاح قیمت «{rows[0]['name']}» به {_money(want)}",
                            "params": {"batch_id": b.id, "sell_price": want}})
            ceiling_note = " (قیمت مصرف‌کننده ثبت نشده — پیش از اعمال، آن را کنترل کنید)"
        elif legal >= want:
            acts.insert(0, {"type": "set_price", "label": f"اصلاح قیمت «{rows[0]['name']}» به {_money(want)}",
                            "params": {"batch_id": b.id, "sell_price": want}})
        else:
            ceiling_note = (f" قیمت مصرف‌کنندهٔ «{rows[0]['name']}» {_money(pricing_law.ceiling(b))} است، "
                            "پس قیمت پیشنهادی بالاتر از آن مجاز نیست و این کالا در این قیمت خرید سود ندارد؛ "
                            + pricing_law.buy_price_is_not_a_lever(b))
            acts.insert(0, {"type": "reorder_note", "label": "ثبت برای بازبینی تأمین/حذف",
    ...
    return [Draft(kind="NEGATIVE_MARGIN", dedupe_key=f"month:{ctx.today.strftime('%Y-%m')}", priority=1,
                  ...
                  body=(f"بعد از احتساب تخفیف‌های پای صندوق، «{rows[0]['name']}» در {_fa(rows[0]['lines'])} فاکتور با زیان فروخته شده ({_money(rows[0]['loss'])}). "
                        f"یا قیمت خرید جدید ثبت نشده، یا قیمت فروش قدیمی است، یا تخفیف دستی زیاد است — در هر سه حالت هر فروش این کالا پول از جیب شماست." + ceiling_note),
...
def a_price_rounding(ctx: Ctx) -> list[Draft]:
    ...
    for b in e.active_batches:
        ...
        r = p % step
        # Rounding up is a price increase in all but name, so it is bound by the same ceiling.
        _ceil = pricing_law.ceiling(b)
        if _ceil is not None and p + (step - r) > _ceil:
            skipped_ceiling += 1
            continue
    ...
    return [Draft(kind="PRICE_ROUNDING", dedupe_key=f"month:{ctx.today.strftime('%Y-%m')}", priority=4,
                  ...
                  body=(f"«{rows[0]['name']}» {_money(rows[0]['price'])} است؛ {_money(rows[0]['new_price'])} برای مشتری فرقی ندارد ولی با {_fa(rows[0]['monthly_units'])} عدد فروش ماهانه {_money(rows[0]['gain'])} اضافه می‌آورد و خرده‌پول صندوق را هم کم می‌کند. "
                        ...
                        + (f" {_fa(skipped_ceiling)} کالا به‌خاطر سقف قیمت مصرف‌کننده گرد نشد." if skipped_ceiling else "")),
...
def a_elasticity(ctx: Ctx) -> list[Draft]:
    ...
    if bad and b:
        _restore = pricing_law.clamp_sell(b, _f(r0["price_before"]))
        acts.append({"type": "set_price", "label": f"بازگرداندن قیمت «{r0['name']}» به {_money(_restore)}", "params": {"batch_id": b.id, "sell_price": _restore}})
...
def a_peak_hours(ctx: Ctx) -> list[Draft]:
    ...
    # A shop that sells in only one or two hours a day (or has no opening-hours setting) leaves
    # this list empty, and open_h[0] raised IndexError — silently, so PEAK_HOURS just never
    # appeared for exactly the small shops that most need to know their quiet hours.
    if not open_h:
        open_h = rows or top
    if not open_h:
        return []
