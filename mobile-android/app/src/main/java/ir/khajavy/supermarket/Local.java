...
public final class Local {
    ...
    static final String[][] DEFAULTS = {
        ...
        {"products.autofill_requires_confirm", "true", "تکمیل خودکار مشخصات باید تأیید شود"}, {"images.auto_find", "true", "یافتن خودکار تصویر کالا (بر اساس نام/برند/بارکد) هنگام ثبت"}, {"images.web_fallback", "true", "در نبود نتیجه از OpenFoodFacts/ویکی‌مدیا، جست‌وجوی تصویر وب بدون کلید (DuckDuckGo) هم استفاده شود"}, {"images.generic_fallback", "true", "اگر عکس بسته‌بندی از فروشگاه‌ها پیدا نشد، از عکس‌های عمومی (ویکی‌پدیا/ویکی‌مدیا) استفاده شود"}, {"images.retail.digikala", "true", "منبع تصویر: دیجی‌کالا (عکس بسته‌بندی، عنوان فارسی)"}, {"images.retail.okala", "false", "منبع تصویر: اُکالا (نیاز به شناسهٔ فروشگاه/توکن — پیش‌فرض خاموش)"}, {"images.retail.basalam", "true", "منبع تصویر: باسلام"}, {"images.retail.torob", "false", "منبع تصویر: ترب (قوانینش استخراج خودکار را توصیه نمی‌کند — پیش‌فرض خاموش)"}, {"pricing.default_margin_percent", "20", "درصد سود پیشنهادی برای قیمت فروش"}, {"pricing.round_to", "1000", "گرد کردن قیمت پیشنهادی"},
    ...
    static Object products(String method, String[] seg, JSONObject b) throws Exception {
        ...
        if (seg.length > 3 && "image".equals(seg[2]) && "candidates".equals(seg[3])) { JSONObject p = Db.productById(Long.parseLong(seg[1])); return obj("product_id", Long.parseLong(seg[1]), "name", p == null ? "" : p.optString("name"), "current", p == null ? null : p.opt("image_url"), "candidates", Images.listCandidates(Long.parseLong(seg[1]))); }
        if (seg.length > 3 && "image".equals(seg[2]) && "pick".equals(seg[3])) { JSONObject rep = Images.setFromUrl(Ui.ctx, Long.parseLong(seg[1]), b.optString("url"), b.optString("source", "picked")); if (!rep.optBoolean("ok")) throw new Api.ApiError(400, "INVALID_IMAGE", "دریافت تصویر ناموفق: " + rep.optString("reason", "")); return rep; }
