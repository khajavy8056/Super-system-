...
public final class Insights {
    ...
    static final String[][] LABELS = {{"CROSS_SELL", "هم‌خرید و چیدمان"}, {"EXPIRY_LADDER", "تخفیف پله‌ای انقضا"}, {"DEAD_STOCK", "سرمایهٔ راکد"}, {"VELOCITY", "هشدار اتمام موجودی"},
        ...
        {"PRICE_GAP", "اصلاح قیمت"}, {"LOSS_PREV", "پیشگیری از ضرر"}, {"SEASON", "الگوی فصلی و هفتگی"}};
    ...
    static List<Draft> velocity(Frame f) throws Exception {
        ...
        for (Map.Entry<Long, Double> e : q28.entrySet()) { long pid = e.getKey(); if (days.get(pid).size() < 6) continue; double v = e.getValue() / 28, st = stock.getOrDefault(pid, 0.0), cover = v > 0 ? st / v : 99; if (cover > 6) continue; double m = f.margin(pid), lost = v * 7 * m; if (lost < 30000) continue; int reorder = (int) Math.ceil(v * 14 - st);
            ...
            d.act("reorder_note", "افزودن به لیست سفارش", "product_id", pid, "qty", Math.max(reorder, 1)).act("set_min_stock", "تنظیم حداقل موجودی هوشمند", "product_id", pid, "min_stock", (int) Math.ceil(v * 5)); d.metric = Local.obj("metric", "stockout_days", "product_id", pid, "window_days", 14, "margin_per_day", Math.round(v * m)); out.add(d); }
    ...
    static JSONObject measure(JSONObject row) throws Exception {
        ...
        double ctrl = 1.0; if (m.matches("product_units|product_profit|customer_sales|attach_rate|receivables_collected|weekday_sales")) { double sb = storeProfitRate(base.optString("from", plusDays(start, -baseDays)), start), sp = storeProfitRate(start, end); if (sb > 0 && sp > 0) ctrl = Math.max(0.5, Math.min(2.0, sp / sb)); }
